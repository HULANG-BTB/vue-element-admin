...
export default {
  ...
  getActiveUser: function () {
    ...
    if (uid) {
      const activeUserStr = this.getUserSession('activeUser')
      return JSON.parse(activeUserStr)
    } else {
  ...
  checkActiveUser: function () {
    ...
    if (jwt_base64) {
      const jwt = Base64.decode(jwt_base64)
      var jwtDecodeVal = jwtDecode(jwt)
      // console.log(jwtDecodeVal);
  ...
  checkAuthorities: function (router, permission) {
    if (permission !== 'Login' && permission !== 'Logout') { // 校验权限
      const activeUser = this.getActiveUser()
      const authorities = activeUser.authorities
      if (!authorities) {
        Message.error('对不起您没有此操作权限！')
        // 跳转到统一授权失败页面
        window.location = 'http://127.0.0.1/#/login?returnUrl=' + Base64.encode(window.location)
        /* router.push({
          path: '/login'
        })*/
      }
  ...
  checkmenu (routeValue, authorities) {
    if (routeValue.meta && routeValue.meta.permission) {
      const permission = routeValue.meta.permission

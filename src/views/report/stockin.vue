...
export default {
  ...
  methods: {
    /* 饼状图的数据填入*/
    async getList () {
      const list = (await getNumIn()).data
      this.setPie(list)
      this.myPie()
    },
    /* 柱状图数据填入*/
    async getBar () {
      const barData = (await getBarIn()).data
      this.myBar(barData)
    },
    myBar (barData) {
